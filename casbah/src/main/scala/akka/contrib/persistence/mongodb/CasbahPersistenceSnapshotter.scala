...
class CasbahPersistenceSnapshotter(driver: CasbahPersistenceDriver) extends MongoPersistenceSnapshottingApi {
  ...
  private[this] implicit def serializeSnapshot(snapshot: SelectedSnapshot): DBObject =
	    MongoDBObject(PROCESSOR_ID -> snapshot.metadata.processorId,
	      SEQUENCE_NUMBER -> snapshot.metadata.sequenceNr,
	      TIMESTAMP -> snapshot.metadata.timestamp,
	      SERIALIZED -> driver.serialization.serializerFor(classOf[SelectedSnapshot]).toBinary(snapshot))
	      
   private[this] implicit def deserializeSnapshot(document: DBObject): SelectedSnapshot = {
  ...
  private[mongodb] def findYoungestSnapshotByMaxSequence(pid: String, maxSeq: Long, maxTs: Long)(implicit ec: ExecutionContext) = Future {
    driver.breaker.withSyncCircuitBreaker {
      snaps.find($and(PROCESSOR_ID $eq pid, SEQUENCE_NUMBER $lte maxSeq, TIMESTAMP $lte maxTs))
  ...
  private[mongodb] def deleteSnapshot(pid: String, seq: Long, ts: Long)(implicit ec: ExecutionContext) = driver.breaker.withSyncCircuitBreaker {
    snaps.remove(
        $and(PROCESSOR_ID $eq pid, SEQUENCE_NUMBER $eq seq, TIMESTAMP $eq ts), 
        WriteConcern.JournalSafe
    )
  }
  
  private[mongodb] def deleteMatchingSnapshots(pid: String, maxSeq: Long, maxTs: Long)(implicit ec: ExecutionContext) = driver.breaker.withSyncCircuitBreaker {
    snaps.remove(
  ...
  private[this] def snaps(implicit ec: ExecutionContext): MongoCollection = {
