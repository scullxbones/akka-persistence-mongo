...
docker run -d -p $MONGODB_NOAUTH_PORT:27017 scullxbones/mongodb:$MONGODB_VERSION --noauth --storageEngine wiredTiger --bind_ip_all
docker run -d -p $MONGODB_AUTH_PORT:27017 scullxbones/mongodb:$MONGODB_VERSION --auth --storageEngine wiredTiger --bind_ip_all
